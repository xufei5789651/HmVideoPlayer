...
int32_t Demuxer::GetTrackInfo(std::shared_ptr<OH_AVFormat> sourceFormat, SampleInfo &info) {
    ...
    for (int32_t index = 0; index < trackCount; index++) {
        ...
        if (trackType == MEDIA_TYPE_VID) {
            ...
            AVCODEC_SAMPLE_LOGI("audioMime:%{public}s audioSampleFormat:%{public}d "
                                "sampleRate:%{public}d channelCount:%{public}d channelLayout:%{public}d",
                                info.audioCodecMime.c_str(), info.audioSampleFormat, info.audioSampleRate,
...
int32_t Demuxer::Seek(int64_t position) {
    int32_t ret = OH_AVDemuxer_SelectTrackByID(demuxer, videoTrackId);
    if (ret != RESULT_CODE_OK) {
        LOGE("Select TrackByID videoTrackId failed");
        return RESULT_CODE_ERROR;
    }

    ret = OH_AVDemuxer_SeekToTime(demuxer, position, SEEK_MODE_NEXT_SYNC);
    if (ret != RESULT_CODE_OK) {
        LOGE("SeekToTime video failed");
        return RESULT_CODE_ERROR;
    }

    ret = OH_AVDemuxer_SelectTrackByID(demuxer, audioTrackId);
    if (ret != RESULT_CODE_OK) {
        LOGE("Select TrackByID audioTrackId failed");
        return RESULT_CODE_ERROR;
    }

    ret = OH_AVDemuxer_SeekToTime(demuxer, position, SEEK_MODE_NEXT_SYNC);
    if (ret != RESULT_CODE_OK) {
        LOGE("SeekToTime audio failed");
        return RESULT_CODE_ERROR;
    }
